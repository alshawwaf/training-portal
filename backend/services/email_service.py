...
logger = logging.getLogger("training_portal.email_service")
...
class EmailService:
    def __init__(self):
        ...
        self.has_custom_certs: bool = False

    def _create_ssl_context(self, settings_map: dict) -> Optional[ssl.SSLContext]:
        """Create custom SSL context with CA and/or client certificates."""
        ca_cert = settings_map.get("smtp_ca_cert")
        client_cert = settings_map.get("smtp_client_cert")
        client_key = settings_map.get("smtp_client_key")
        key_password = settings_map.get("smtp_key_password")
        validate_certs = settings_map.get("smtp_validate_certs", "false").lower() == "true"
        
        # If no custom certs, return None (use default behavior)
        ...
        try:
            ...
            if ca_cert:
                ...
                try:
                    ...
                    logger.info("Custom CA certificate loaded for SMTP")
                finally:
                    os.unlink(ca_path)
            ...
            if client_cert and client_key:
                ...
                try:
                    ...
                    logger.info("Client certificate loaded for mTLS authentication")
                finally:
                    os.unlink(cert_path)
                    os.unlink(key_path)
            
            self.has_custom_certs = True
            return context
    ...
    async def load_config(self, db: Session):
        ...
        try:
            ...
            # Create custom SSL context if certificates are configured
            self.ssl_context = self._create_ssl_context(settings_map)
            ...
            cert_info = ""
            if self.has_custom_certs:
                cert_info = " (with custom certificates)"
            logger.info(f"Email configuration loaded successfully. Server: {settings_map['smtp_server']}:{settings_map['smtp_port']}{cert_info}")
        except Exception as e:
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 520px; margin: 0 auto;">
                            ...
                            <table role="presentation" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td style="background: linear-gradient(135deg, #3b82f6 0%, #6366f1 100%); padding: 12px 16px; border-radius: 12px;">
                                        <span style="color: #ffffff; font-size: 20px; font-weight: 700; letter-spacing: -0.5px;">Training Portal</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                                        ...
                                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #334155;">
                                            <tr>
                                                <td>
                                                    ...
                                                    <p style="margin: 4px 0 0 0; font-size: 14px; font-weight: 600; color: #e2e8f0;">Training Portal Team</p>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                    ...
                    <tr>
                        <td align="center" style="padding: 24px 0;">
                            <p style="margin: 0 0 8px 0; font-size: 12px; color: #475569;">This is an automated notification from Training Portal</p>
                            <p style="margin: 0; font-size: 12px; color: #475569;">&copy; {datetime.datetime.now().year} <span style="font-weight: 600; color: #64748b;">Training Portal</span></p>
                        </td>
                    </tr>
                </table>
            </td>
