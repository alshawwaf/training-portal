            ...
            {stats && activeTab === 'operational' && (
                <div className="flex items-center gap-4 text-sm">
                    ...
                    {Object.entries(stats.by_source || {}).filter(([_, count]) => count > 0).map(([source, count]) => (
                        <div key={source} className="flex items-center gap-1 px-2 py-0.5 bg-secondary/20 rounded text-xs">
                            <span className={sourceConfig[source]?.color || 'text-secondary'}>{sourceConfig[source]?.label || source}</span>
